...
def add_metadata_table(doc):
    ...
    rows = [
        ...
        ("设计规模", "约 250–300 人会议", "当前状态", "完成部署与小范围内测"),
...
def build():
    ...
    add_callout(
        doc,
        "事实边界",
        "系统按约 250–300 人会议规模设计并完成部署与小范围内测；现有材料不能证明已有 200+ 名用户正式使用，文档和仓库均不包含真实报名数据。",
    )
    ...
    add_body(
        doc,
        "在完成服务器部署后，项目开展小范围内测并记录 8 项反馈，涉及手机端阅读、付款凭证引导、主题一致性和权限问题。日志保留了优先级排序、跨文件任务清单、回退和复测过程，可以追溯产品与工程决策。",
    )
    add_callout(
        doc,
        "测试口径",
