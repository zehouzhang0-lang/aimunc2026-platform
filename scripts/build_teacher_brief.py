...
def add_metadata_table(doc):
    ...
    rows = [
        ...
        ("设计规模", "约 250–300 人会议", "后续使用", "社团接管后服务 237 名代表"),
        ("角色体系", "代表 / 领队 / 管理员", "技术栈", "Java + Spring Boot + MySQL + 原生 Web"),
        ("交付结果", "人民币 1,500 元一次性有偿交付", "责任边界", "交付后上线运营由社团负责"),
...
def add_evidence_table(doc):
    ...
    rows = (
        ...
        ("有偿交付", "用户确认", "人民币 1,500 元一次性交付；脱敏凭证待补"),
        ("237 名代表", "用户确认", "社团接管后实际服务；待补脱敏聚合证据"),
...
def build():
    ...
    add_callout(
        doc,
        "事实边界",
        "系统按约 250–300 人会议规模设计，早期版本通过小范围测试收集反馈，并以人民币 1,500 元向社团完成一次性有偿交付。据负责人确认，社团接管后的平台实际服务 237 名代表。金额和人数均待脱敏佐证；交付后上线运营与持续运维不在本人控制范围。",
    )
    ...
    add_body(
        doc,
        "这是我的第一个独立全栈项目。除业务规则、运营需求和验收反馈与社团讨论外，我是唯一的人类开发者，独立承担产品梳理、数据建模、前后端实现、管理端设计、交付前测试、初始部署和技术交接。Claude 用于架构讨论、编码辅助和审计；平台本身没有内置 Agent 功能。",
    )
    ...
    responsibilities = (
        ...
        ("测试与交付：", "完成手工 API/E2E 联调、问题复现和回归，使用 VPS、Cloudflare、Nginx 与 systemd 完成初始部署，并向社团完成技术交接。"),
    )
    ...
    add_body(
        doc,
        "交付前完成初始服务器部署，早期版本通过小范围测试记录 8 项反馈，涉及手机端阅读、付款凭证引导、主题一致性和权限问题。项目随后以人民币 1,500 元向社团完成一次性有偿交付；据负责人确认，社团接管后的平台实际服务 237 名代表。",
    )
    add_callout(
        doc,
        "测试口径",
    ...
    add_body(
        doc,
        "Claude Opus/Sonnet 在研发过程中曾充当架构讨论者和代码助手，参与任务拆解、实现建议、错误定位、跨文件审计和开发日志交接。我负责决定需求是否进入产品、选择或拒绝方案、整合代码、运行验证、初始部署，并对约定交付范围负责；交付后的生产运营由社团负责。",
    )
    add_body(
        doc,
    ...
    limitations = (
        ...
        "237 名代表与人民币 1,500 元交付金额均待补脱敏佐证；前者不等于并发或性能验证，后者不代表持续营收或知识产权转让。",
        "交付后的生产环境由社团控制，本人无法复核其持续运维、监控、峰值流量或长期稳定性。",
    ...
    add_body(
        doc,
        "我选择保留这些不足，因为第一版的价值不是证明它已经完美，而是证明我能够把一个实际组织问题推进到可运行、可部署、可付费交付的系统，明确交接后的责任边界，并在事后识别结构性问题。",
    )
    ...
    add_body(
        doc,
        "AIMUNC 2026 让我第一次完整经历“理解真实需求—构建产品—完成有偿工程交付—明确责任移交—反思并规划重构”的过程。它连接了我希望继续发展的三个方向：AI 产品经理需要把复杂运营问题转化为清晰产品边界；Agent 应用开发需要寻找可验证、可控的真实任务；FDE 需要深入业务现场，与使用者协作并独立推动方案落地。",
    )
